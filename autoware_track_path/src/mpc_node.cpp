...
#include <Eigen/Core>
#include <Eigen/QR>
#include "autoware_track_path/mpc.h"
...
private:
    ...
    double _mpc_steps, _ref_cte, _ref_epsi, _ref_vel, _w_cte, _w_epsi, _w_vel, _w_delta, _w_accel, _w_delta_d, _w_accel_d, _max_steering,
        _max_throttle, _bound_value;
...
MPCNode::MPCNode() {
    ...
    if (_pub_twist_flag) _pub_twist = _nh.advertise<geometry_msgs::Twist>("/cmd_vel", 1); // for stage (Ackermann msg non-supported)
...
int MPCNode::get_thread_numbers() { return _thread_numbers; }
...
void MPCNode::odomCB(const nav_msgs::Odometry::ConstPtr& odomMsg) { _odom = *odomMsg; }
...
void MPCNode::pathCB(const nav_msgs::Path::ConstPtr& pathMsg) {
    if (_goal_received && !_goal_reached) {
        ...
        try {
            ...
            // DEBUG
            // cout << endl << "N: " << odom_path.poses.size() << endl <<  "Car path[0]: " << odom_path.poses[0] << ", path[N]: " <<
            // _odom_path.poses[_odom_path.poses.size()-1] << endl;
...
void MPCNode::controlLoopCB(const ros::TimerEvent&) {
    if (_goal_received && !_goal_reached && _path_computed) // received goal & goal not reached
    {
        ...
        _throttle = 0.0;
        _speed = 0.0;
        if (_goal_reached && _goal_received) cout << "Goal Reached !" << endl;
    }
